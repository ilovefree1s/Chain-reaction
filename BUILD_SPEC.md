...
```json
{
  ...
  "wheelExcludes": [7, 8, 11, 12, 13, 15, 18, 19, 27, 31, 36, 39, 43, 48],
  "timings": [
    "Before shot",
    "Before tee shot",
    "After throw",
  ...
  "cards": [
    ...
    { "id": 2, "timing": "Before shot", "kind": "attack", "name": "Tomahawk", "text": "Force an opponent to throw a tomahawk on the upcoming drive or approach." },
    ...
    { "id": 4, "timing": "Before shot", "kind": "attack", "name": "Choose Disc", "text": "Choose ANY disc from any bag. That disc must be used on their next shot." },
    { "id": 5, "timing": "Before shot", "kind": "attack", "name": "Straddle Putt", "text": "Force an opponent to straddle putt. Playable any time someone is about to throw a putter." },
    { "id": 6, "timing": "Before shot", "kind": "attack", "name": "Mini Drive", "text": "Force an opponent to throw a mini for their upcoming drive or putt. Through the basket floor still counts as in." },
    { "id": 7, "timing": "After throw", "kind": "attack", "name": "nnnnNunan!", "text": "Heckle an opponent during one of their shots. Reveal and discard this card after." },
    { "id": 8, "timing": "For the next hole", "kind": "attack", "name": "Caddy", "text": "Force an opponent to carry your discs until they beat you on a hole. While they carry your bag, you can't play any more cards on them." },
    { "id": 9, "timing": "After throw · secret", "kind": "self", "name": "Don't Nice Me!", "text": "If any player says \"nice\" as your disc is in flight and it hits a tree, that throw doesn't count. Move to your disc and throw again." },
    ...
    { "id": 13, "timing": "Before tee shot", "kind": "attack", "name": "Unfamiliar Discs", "text": "You pick the discs every other player tees off with this hole." },
    { "id": 14, "timing": "Before tee shot", "kind": "attack", "name": "Bizarro Hole Solo", "text": "Force an opponent to drive with a putter and putt with a driver this hole." },
    { "id": 15, "timing": "After throw", "kind": "attack", "name": "Cancel Shot", "text": "Cancel a shot an opponent just took. They throw again with a different disc of their choice. No extra stroke." },
    { "id": 16, "timing": "Before shot", "kind": "attack", "name": "Blind Putt", "text": "Force an opponent to take the next putt with their eyes closed." },
    { "id": 17, "timing": "Before tee shot", "kind": "attack", "name": "One Disc Hole", "text": "Force an opponent to play the hole with only 1 disc. You choose it." },
    ...
    { "id": 19, "timing": "After all tee", "kind": "self", "name": "Lie Swap", "text": "After all tee shots, trade lies with an opponent of your choice." },
    { "id": 20, "timing": "Before tee shot", "kind": "self", "name": "1v1 Challenge", "text": "Challenge someone. First player to get a better score on a hole gets a free immunity hole and 1 free mulligan on that hole." },
    { "id": 21, "timing": "Before tee shot", "kind": "attack", "name": "Ambidextrous Drive", "text": "Force an opponent to take the upcoming drive with their off hand." },
    { "id": 22, "timing": "Before tee shot", "kind": "attack", "name": "Create Tee", "text": "Use 2 of your discs to build a new tee for every opponent. Within 10 paces (30 ft) of the original." },
    { "id": 23, "timing": "Before tee shot", "kind": "self", "name": "Create Tee Solo", "text": "Use 2 of your discs to build a new tee for yourself. Within 10 paces (30 ft) of the original. You may pick 1 person to join you." },
    { "id": 24, "timing": "Before tee shot", "kind": "attack", "name": "Thief'n", "text": "Steal someone's disc. It's yours for the rest of the round." },
    { "id": 25, "timing": "After throw", "kind": "attack", "name": "Bad Lie", "text": "Move an opponent's lie 10 paces (30 ft) in any direction, as long as it isn't out of bounds." },
    { "id": 26, "timing": "After throw", "kind": "self", "name": "Better Lie", "text": "Move your own lie 10 paces (30 ft) in any direction." },
    { "id": 27, "timing": "After card", "kind": "react", "name": "No Way", "text": "Cancel any card just played. That card goes to the discard pile." },
    { "id": 28, "timing": "Before tee shot", "kind": "attack", "name": "Standstill", "text": "No run-up on an opponent's next throw." },
    { "id": 29, "timing": "Before shot", "kind": "attack", "name": "Turbo Time", "text": "An opponent's next putt must be a turbo putt." },
    { "id": 30, "timing": "Before tee shot", "kind": "attack", "name": "Mando Maker", "text": "Choose a reasonable object the target player must pass on a side you specify." },
    ...
    { "id": 33, "timing": "Before tee shot", "kind": "self", "name": "Call Your Shot", "text": "Call CTP. If you win it, nobody can play cards on you next hole, and your next card affects everyone even if it doesn't say so." },
    ...
    { "id": 35, "timing": "After a hole", "kind": "attack", "name": "FIVE DOLLARS!", "text": "Play on a player who failed to birdie a hole every other player birdied. Five dollars in the pot, and they only draw 1 card next turn." },
    ...
    { "id": 37, "timing": "Before tee shot", "kind": "self", "name": "Clean Slate", "text": "Remove any and all card effects currently on you." },
    { "id": 38, "timing": "Before tee shot", "kind": "self", "name": "Trade Offer", "text": "Ask another player to team up for the next hole. Birdie counts as an eagle, no birdie is +1. Cards against one of you count against both. They may refuse, then you ask someone else. You both take the same score." },
    { "id": 39, "timing": "After all play a card", "kind": "group", "name": "Group Hug", "text": "Only playable after every other player has played a negative card this turn. Each of them carries out their own card instead of the player they aimed it at." },
    { "id": 40, "timing": "Before shot", "kind": "self", "name": "Metal Counts", "text": "Play before a C2 putt only. If you hit metal and it doesn't go in, it still counts." },
    ...
    { "id": 43, "timing": "Before tee shot", "kind": "group", "name": "RUSH ATTACK!", "text": "Everyone throws from anywhere near the tee pad. No throw order, rush ready golf. First to finish gets birdie, 2nd and 3rd get par, last gets bogey. The catch: NO RUNNING." },
    { "id": 44, "timing": "After throw", "kind": "attack", "name": "Walk of Shame", "text": "After a missed putt inside C1, that player carries their putter in their off hand until the next tee." },
    ...
    { "id": 46, "timing": "Before shot", "kind": "dual", "name": "Behind the Back Putt", "text": "On yourself: putt behind the back at no stroke cost. On another player: they putt behind the back and it counts as a normal stroke." },
    ...
    { "id": 48, "timing": "Before tee shot", "kind": "group", "name": "Double Wheel", "text": "Spin a random card effect and reveal it, then spin the name wheel. That player is the only one it doesn't apply to. Everyone else, including you, has to do it." },
    ...
    { "id": 50, "timing": "Any time", "kind": "dual", "name": "Do-Over", "text": "On yourself: retake any shot for free. On another player: force them to re-throw a shot that was too good." },
    { "id": 51, "timing": "After a hole", "kind": "dual", "name": "Me and You", "text": "On yourself: tie the best score on the hole for free. On another player: they tie the worst score on the hole." },
...
- Spin the Double Wheel 30 times → no blacklisted card (7, 8, 11, 12, 13, 15, 18, 19, 27, 31, 36, 39, 43, 48) ever appears.
