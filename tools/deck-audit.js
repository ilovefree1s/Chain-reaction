...
const SETTLED = {
  ...
  50: "Dual, unchanged targeting. Reworded to match Lefty ON the Box: second person, and the player in last place gets a gimme too, rock paper scissors settling a tie for last.",
  ...
  59: "Added 2026-08-26 by the user's design. A reaction like No Way: plays with no target — whoever it bounces back at is obvious at the table. Doesn't work on wheel spins by its own text.",
  60: "Added 2026-08-26 by the user's design. The first WHEEL-ONLY card: never shuffled into a deck, so it has no play targeting at all — it exists purely as a wheel landing.",
  61: "Added 2026-08-28 by the user's design, promoted to a regular deck card 2026-08-29 as PUT EM ON TILT: in hand it's an attack you aim at an opponent; off the wheel it still lands on whoever bought the spin.",
  62: "Added 2026-08-29 by the user's design, cut from the same cloth as PUT EM ON TILT: an attack you aim at an opponent, who plays the entire hole with the slightly used Anax.",
...
const WHEEL_SETTLED = {
  ...
  35: "Stays off the wheel. Playable from a hand only.",
  39: "Stays off the wheel. Playable from a hand only.",
  14: "Stays off the wheel. Playable from a hand only.",
  ...
  59: "Stays off the wheel, like the other reaction cards — it has nothing to react to when drawn this way.",
  60: "WHEEL-ONLY, in the user's words. The wolf isn't sitting anything out, so it reads as its own text with no exempt banner (SAYS_IT_ALL).",
  61: "On the wheel it keeps its own words with [spinner] filled in: “[spinner] spun the wheel and has to play the entire hole with the TILT!” No exempt banner (SAYS_IT_ALL) — the spun name has nothing to do with this one.",
  62: "On the wheel it lands like PUT EM ON TILT: “[spinner] spun the wheel and has to play the entire hole with the... slightly used Anax...” No exempt banner (SAYS_IT_ALL).",
  ...
  50: "On the wheel: everyone but the exempt player gets one C1 gimme this hole. The last-place rider is a hand-version thing and stays there.",
  ...
  34: "Renamed Lefty Off the Box → \"Lefty ON the Box\". On the wheel it splits like Globetrotter: the spun name gets the offhand throw free as throw 0, everyone else throws lefty and wears it as throw 1.",
  30: "Joins the wheel with a mechanic of its own: after the card lands the wheel spins a SECOND name, and those two play the hole as a team against everybody else. Birdie counts as an eagle, no birdie or better is +1 stroke after the hole, cards on either teammate count against both. The refuse-and-everyone-discards clause is gone from the card entirely. 2026-08-29: the winning team banks a mulligan each (duo's score vs. the other side's best, tie = no prize) — the user wanted the win itself to pay.",
  29: "On the wheel it stands alone: no header over it, no summing-up under it. The card names the spun player, says everyone else still putts behind the back for a regular stroke, and pins it to Circle 1.",
